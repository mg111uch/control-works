...
class Mana(Component):
    def __init__(self, max_mana: float):
        self.current = max_mana
        self.maximum = max_mana
...
class Inventory(Component):
    def __init__(self):
        self.slots: List[Optional[Dict]] = [None] * 6
        self.gold = 600
...
class Hero(Entity):
    def __init__(self, model: "GameModel", player_id: int, team: int, position, data: dict):
        ...
        self.add_component("buffs", type("Buffs", (), {"active": {}}))  # For BKB/invis
    ...
    def cast_raze(self, which: str):
        ...
        proj = RazeProjectile(self.model, pos + direction * 100, direction, ranges[which], self)
    ...
    def cast_requiem(self):
        ...
        pos = self.get_component("position").value
        souls = self.get_component("souls").current
        damage_per_line = 80 + souls * 0.4

        for i in range(12):
            angle = i * np.pi / 6
            direction = np.array([np.cos(angle), np.sin(angle)])
            line = RequiemLine(self.model, pos, direction, damage_per_line, self)
    ...
    def get_total_stats(self):
        ...
        base_str = self.data["base_stats"]["strength"] + (self.level - 1) * self.data["base_stats"]["strength_gain"]
        # Full calc...
        return stats