...
def serialize_input_packet(tick: int, input_data: Dict[str, Any]) -> bytes:
    ...
    payload = {
        "tick": tick,
        "move": input_data["move_target"].tolist() if input_data["move_target"] is not None else None,
        "attack_move": bool(input_data["attack_move"]),
        "raze": input_data["cast_raze"],
        "requiem": bool(input_data["cast_requiem"]),
        "item": input_data["use_item"],
        "stop": bool(input_data["stop"])
    }
...
def deserialize_input_packet(data: bytes) -> Dict[str, Any]:
    ...
    try:
        ...
        if move:
            payload["move"] = np.array(move, dtype=np.float32)
        return payload
    except:
        return None
